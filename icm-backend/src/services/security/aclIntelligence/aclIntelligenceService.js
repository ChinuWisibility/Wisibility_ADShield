...
/**
 * Shadow admin detection — graph heuristics + ACL-derived privilege paths (v2).
 */
export async function detectShadowAdmins(ctx) {
  ...
  const merged = [...graphFindings, ...(aclResult.findings || [])];
  return { feature: "shadow_admins", count: merged.length, findings: merged };
}
