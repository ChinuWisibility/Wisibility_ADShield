...
import {
  Groups3 as RolesIcon,
  WorkspacePremium as RoleMiningIcon,
...
/** Role Management sub-tabs (one page): Roles | Role Mining */
export const ROLE_SUMMARY_SUB_TABS = [
  {
    id: 'roles',
    label: 'Roles',
    path: '/access/roles',
  ...
  },
  {
    id: 'role-mining',
    label: 'Role Mining',
    path: '/access/role-mining',
    icon: RoleMiningIcon,
  },
];

function roleSummaryTabIndex(pathname) {
  if (pathname.includes('role-mining')) return 1;
  return 0;
}

/**
 * Browser-tab style switcher with Chrome scoop curves (same as Correlation Summary).
 */
function RoleSummarySubTabs({ tabs, value, onChange }) {
  const scoop = 12;
...
/**
 * Role Management hub — sidebar: Role Management; page sub-tabs: Roles | Role Mining.
 */
export default function RoleSummaryHub() {
  const location = useLocation();
  const navigate = useNavigate();
  const tabIndex = useMemo(
    () => roleSummaryTabIndex(location.pathname),
    [location.pathname],
  );
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <RoleSummarySubTabs
        tabs={ROLE_SUMMARY_SUB_TABS}
        value={tabIndex}
        onChange={(_e, next) => {
          const tab = ROLE_SUMMARY_SUB_TABS[next];
